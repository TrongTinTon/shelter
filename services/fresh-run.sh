...
docker service rm vault_vault-1 vault_vault-2 vault_vault-transit-1 || echo "Services not created yet"
...
docker volume rm vault_vault-1-data vault_vault-2-data vault_vault-transit-1-data || echo "Volumes not created yet"
docker rmi vault-1 vault-2 vault-transit-1 --force || echo "Images not created yet"
